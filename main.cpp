...
// Create an UnbufferedSerial object for USB serial communication
    UnbufferedSerial serial(USBTX, USBRX, 9600);
 void on_serial_rx() {
 
    // Read the received character
    serial.read(&received_char, 1);
    new_data = true;
    }
    ...
    while (true) {
        if (new_data) {
            ...
            switch (received_char) {
                ...
                case 'h':
                case 'H':
                    printf("Humidity and Temperature data:\r\n");
                    print_t_rh();
                    break;

                case 'p':
                case 'P':
                    printf("Pressure and Temperature data:\r\n");
                    print_t_rh();
                    break;
